...
import { ScoreBreakdown, SeasonResults, UserScore } from '../models/season-result.model';
...
export class ScoresService {
  ...
  calculateScores(predictions: UserPrediction[], realWinners: SeasonResults): UserScore[] {
    const scores = predictions.map((prediction) => {
      ...
      breakdown.push({
        label: 'LaLiga',
        points: laLigaCorrect ? this.SCORE_RULES.laLigaWinner : 0,
      ...
      breakdown.push({
        ...
        points: championsCorrect ? this.SCORE_RULES.championsLeagueWinner : 0,
      ...
      breakdown.push({
        label: 'Copa del Rey',
        points: copaReyCorrect ? this.SCORE_RULES.copaReyWinner : 0,
      ...
      breakdown.push({
        label: 'Supercopa',
        points: superCopaCorrect ? this.SCORE_RULES.superCopaWinner : 0,
      ...
      const topScorerCorrect = realWinners.topScorer
        ? this.isCorrectPrediction(prediction.seasonPrediction.topScorer, realWinners.topScorer)
        : null;

      breakdown.push({
        label: 'Pichichi',
        points: topScorerCorrect ? this.SCORE_RULES.topScorer : 0,
      ...
      breakdown.push({
        ...
        points: standOutPlayerCorrect ? this.SCORE_RULES.standOutPlayer : 0,
      ...
      const disappointmentPlayerCorrect = realWinners.disappointmentPlayer
        ? this.isCorrectPrediction(
      ...
      breakdown.push({
        ...
        points: disappointmentPlayerCorrect ? this.SCORE_RULES.disappointmentPlayer : 0,
      ...
      const ballonDorCorrect = realWinners.ballondOr
        ? this.isCorrectPrediction(prediction.seasonPrediction.ballondOr, realWinners.ballondOr)
        : null;

      breakdown.push({
        label: 'Balón de Oro',
        points: ballonDorCorrect ? this.SCORE_RULES.ballondOr : 0,
      ...
      const goldenBootCorrect = realWinners.goldenBoot
        ? this.isCorrectPrediction(prediction.seasonPrediction.goldenBoot, realWinners.goldenBoot)
        : null;

      breakdown.push({
        label: 'Bota de Oro',
        points: goldenBootCorrect ? this.SCORE_RULES.goldenBoot : 0,
      ...
      breakdown.push({
        label: 'Trofeo Zamora',
        points: zamoraCorrect ? this.SCORE_RULES.zamoraWinner : 0,
    ...
    return scores.sort((firstScore, secondScore) => secondScore.points - firstScore.points);
  }

  private isCorrectPrediction(predicted: Team | Player, result: Team | Player | null): boolean {
